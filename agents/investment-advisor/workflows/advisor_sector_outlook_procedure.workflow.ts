...
const PROCEDURE = "advisor_sector_outlook_procedure" as const;
...
patch({
  progress: "正在理解行业前景问题",
  state: {
    status: AdvisorStatusSchema.describe("Only set cancelled when the user explicitly stops this 行业前景 workflow; otherwise set researching when the latest message asks or changes this procedure's research request."),
  ...
  instruction: `
本 workflow 只处理行业前景、市场规模、发展趋势、投资逻辑、竞争态势。
抽取行业、板块或主题到 topic/targets；targetKind 设为 industry 或 sector。
...
effect("resolveResearchReadiness", ["status", "targets", "topic"], {
  description: "行业前景 workflow 根据已抽取的研究对象和主题判断是否具备资料收集条件；不调用外部工具。",
  run: (state) => {
...
effect("collectResearchEvidence", [
  ...
  description: "行业前景 workflow 调用本 procedure 需要的只读投研工具；工具结果只通过 ToolMessage 暴露给 render，不写长期 state。",
  run: async (state, context, _runtime, step) => {
    if (state.status !== "researching" || readinessBlocker(state) !== null) return {};

    const loading = step.start("收集行业前景资料");
...
回复顺序应覆盖行业前景、需求驱动、供给/技术约束、竞争格局、投资逻辑、关键风险。
代表公司只能在这些分析之后补充，不能把行业前景问题改写成公司清单。
不能把长期前景等同于相关股票短期上涨。
...
/**
 * Collects read-only evidence for the 行业前景 procedure.
...
function collectEvidence(state: AdvisorState, context: AdvisorContext): Promise<ToolMessage[]> {
  ...
  return evidence.collectAll([
    ...
    evidence.callNews(context, topic + " 行业 前景 发展趋势", 10),
    evidence.callReports(context, topic + " 行业 前景 市场规模 竞争格局 投资逻辑", null, [topic]),
  ]);
}
