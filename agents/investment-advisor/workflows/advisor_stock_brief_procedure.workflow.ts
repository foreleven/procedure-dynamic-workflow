...
patch({
  ...
  instruction: `
本 workflow 只处理裸股票名、股票代码，以及“分析一下”“怎么样”这类泛化股票分析请求。
抽取股票名称或代码到 targets；targetKind 设为 stock。用户只给股票名时不要补代码。
...
必须覆盖公司或业务基本情况、行情、技术、资金线索、近期信息、主要风险和可继续深挖问题。
用户只给股票名且没有结构化行情或公司资料时，要说明缺少完整代码会影响精确性。
不能把“怎么样”直接回答成买入或卖出建议。
用户只给股票名且没有完整代码或结构化行情工具事实时，不能输出精确股价、交易日期、涨跌幅、支撑位、压力位、机构持仓数量等具体数字；只能用定性描述，并说明补充完整代码后可查询。
用户只给股票名时，也不要输出精确业务销量、海外目标、增持金额、车型数量、年份目标、产能数量等运营数字，除非工具事实明确返回并能标注为公开资料或第三方观点。
如果引用机构评级或目标价，必须明确这是第三方观点，不代表本系统建议，也不能把“买入评级”改写成对用户的买入建议。
...
/**
 * Collects read-only evidence for the 股票简析 procedure.
...
function collectEvidence(
  ...
  return evidence.collectAll([
    ...
    ...evidence.queryEvidence(context, state, target.display),
  ]);
}
