...
const PROCEDURE = "advisor_stock_reason_procedure" as const;
...
const metadata = {
  id: PROCEDURE,
  version: "1.0.0",
  description: "投资顾问股票异动原因研究，处理涨跌原因涨停跌停异动啥情况怎么了。",
  routing: defineRouting({
    examples: ["300322涨停原因","工业富联今天为什么大涨","这只股票啥情况"],
    entities: ["涨停","跌停","涨跌原因","为什么大涨","为什么大跌","异动","啥情况","怎么了"],
...
patch({
  progress: "正在理解股票异动问题",
  state: {
    status: AdvisorStatusSchema.describe("Only set cancelled when the user explicitly stops this 股票异动原因 workflow; otherwise set researching when the latest message asks or changes this procedure's research request."),
  ...
  instruction: `
本 workflow 只处理涨跌原因、涨停原因、跌停原因、异动、“啥情况”“怎么了”等问题。
抽取股票到 targets；如果用户表达涨停、跌停、上涨、下跌或异动，topic 可记录该事件语义；targetKind 设为 stock。
...
effect("resolveResearchReadiness", ["status", "targets", "topic"], {
  description: "股票异动原因 workflow 根据已抽取的研究对象和主题判断是否具备资料收集条件；不调用外部工具。",
  run: (state) => {
...
effect("collectResearchEvidence", [
  ...
  description: "股票异动原因 workflow 调用本 procedure 需要的只读投研工具；工具结果只通过 ToolMessage 暴露给 render，不写长期 state。",
  run: async (state, context, _runtime, step) => {
    if (state.status !== "researching" || readinessBlocker(state) !== null) return {};

    const loading = step.start("收集股票异动原因资料");
...
export default render({
  ...
  progress: "正在生成异动原因回复",
  instruction: `
本 workflow 的回复目标是解释可能驱动并标明证据等级。
必须分清工具资料能支持的因素、可能相关但需核验的因素、暂时无法确认的因素。
不能把猜测说成事实，也不能把单一新闻直接说成涨跌唯一原因。
用户问“涨停原因”“跌停原因”时，优先围绕其询问的异动事件解释可能近因；如果当前行情快照没有显示涨停/跌停，只能说明“当前快照未必对应用户所问交易日”，不要把回复重心转成“今天没涨停”。
涉及涨停、追高、概念炒作或异动延续时，必须明确提示短线波动和追高风险，不能暗示异动会持续或反转。
风险提示不能只放在文末；在解释涨停/跌停/异动原因的主体段落中，也要明确写出“短线异动不代表后续延续，追高风险较高”。
...
/**
 * Collects read-only evidence for the 股票异动原因 procedure.
...
function collectEvidence(state: AdvisorState, context: AdvisorContext): Promise<ToolMessage[]> {
  ...
  return evidence.collectAll([
    ...
    evidence.callTopMovers(context, { setDomain: null, wantNum: 20, target: "0", sortType: null }),
    evidence.callNews(context, target.display + " 涨跌 异动 原因", 10),
    evidence.callReports(context, target.display + " 异动 原因", evidence.symbolsFor([target]), null),
  ]);
}
