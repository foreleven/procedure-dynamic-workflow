...
import type { InvestmentAdvisorConnectorCatalog } from "../connectors/main.js";

const PROCEDURE = "advisor_market_index_procedure" as const;
...
const metadata = {
  id: PROCEDURE,
  version: "1.0.0",
  description: "投资顾问市场指数研究，处理 A 股港股美股大盘今日昨日指数行情。",
  routing: defineRouting({
    examples: ["今日大盘","A股今天怎么样","港股指数昨天表现"],
    entities: ["大盘","指数","A股","港股","美股","今日市场"],
...
patch({
  progress: "正在理解市场指数问题",
  state: {
    status: AdvisorStatusSchema.describe("Only set cancelled when the user explicitly stops this 市场指数分析 workflow; otherwise set researching when the latest message asks or changes this procedure's research request."),
  ...
  instruction: `
本 workflow 只处理 A 股、港股、美股大盘，以及今日、昨日或近期指数行情分析。
抽取市场或指数到 targets/topic，时间到 horizon/dateRange；targetKind 设为 market。用户没说明市场时可按 A 股大盘理解。
...
effect("resolveResearchReadiness", ["status", "targets", "topic"], {
  description: "市场指数分析 workflow 根据已抽取的研究对象和主题判断是否具备资料收集条件；不调用外部工具。",
  run: (state) => {
...
effect("collectResearchEvidence", [
  ...
  description: "市场指数分析 workflow 调用本 procedure 需要的只读投研工具；工具结果只通过 ToolMessage 暴露给 render，不写长期 state。",
  run: async (state, context, _runtime, step) => {
    if (state.status !== "researching" || readinessBlocker(state) !== null) return {};

    const loading = step.start("收集市场指数分析资料");
...
export default render({
  ...
  progress: "正在生成市场指数回复",
  instruction: `
本 workflow 的回复目标是概述市场强弱、资金面、热点方向、可能驱动和风险。
没有具体指数代码或结构化行情时，必须定性分析，不能编造指数点位、涨跌幅、成交额、外盘收盘数据或“4000点”等精确点位叙述。
用户只说“大盘”“今日大盘”“A股怎么样”且没有给指数代码时，即使新闻或研报片段出现指数点位，也不能把这些点位写成当前大盘结论；只能说可得资料显示市场线索偏强/偏弱/分化，并说明精确点位需要具体指数代码和结构化行情支持。
海外市场要说明资料时点可能与本地交易时间不同。

本 workflow 的回复边界：
- 如果资料不足或工具失败，说明当前只能给有限分析，并告诉用户补充什么信息可继续推进。
- 区分“工具资料显示”“可能原因/推断”“仍需核验”，不能补写工具没有返回的精确市场、财务、公告或公司关系数据。
- 禁止输出“上证指数重回/跌破某点位”“指数收于某点”“成交额多少亿”等精确市场数字，除非用户给了具体指数代码且工具事实明确返回对应结构化行情。
...
/**
 * Collects read-only evidence for the 市场指数分析 procedure.
...
function collectEvidence(state: AdvisorState, context: AdvisorContext): Promise<ToolMessage[]> {
  const target = evidence.normalizedTargets(state)[0];
  return evidence.collectAll([
    ...(target ? evidence.quoteEvidence(context, target) : []),
    evidence.callTopMovers(context, { setDomain: null, wantNum: 20, target: evidence.marketTargetForState(state), sortType: null }),
    evidence.callNorthbound(context),
    evidence.callSouthbound(context),
    evidence.callNews(context, state.topic ?? "今日 大盘 指数 行情", 10),
    evidence.callReports(context, state.topic ?? "大盘 指数 市场 分析", null, null),
  ]);
}
