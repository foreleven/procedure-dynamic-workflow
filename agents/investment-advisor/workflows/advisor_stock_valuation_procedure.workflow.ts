...
const PROCEDURE = "advisor_stock_valuation_procedure" as const;
...
patch({
  progress: "正在理解股票估值问题",
  state: {
    status: AdvisorStatusSchema.describe("Only set cancelled when the user explicitly stops this 股票估值与投资价值 workflow; otherwise set researching when the latest message asks or changes this procedure's research request."),
  ...
  instruction: `
本 workflow 只处理投资价值、估值、股价合理性、内在价值、贵不贵等问题。
抽取股票到 targets，估值关注点到 topic 或 comparisonFocus；targetKind 设为 stock。
...
effect("resolveResearchReadiness", ["status", "targets", "topic"], {
  description: "股票估值与投资价值 workflow 根据已抽取的研究对象和主题判断是否具备资料收集条件；不调用外部工具。",
  run: (state) => {
...
effect("collectResearchEvidence", [
  ...
  description: "股票估值与投资价值 workflow 调用本 procedure 需要的只读投研工具；工具结果只通过 ToolMessage 暴露给 render，不写长期 state。",
  run: async (state, context, _runtime, step) => {
    if (state.status !== "researching" || readinessBlocker(state) !== null) return {};

    const loading = step.start("收集股票估值与投资价值资料");
...
必须覆盖业务质量、行业位置、景气度、市场表现、资金面、估值方法适用性和风险因素。
没有工具资料明确提供估值指标、目标价或价格判断时，只能说明估值资料缺口，不能自创内在价值或目标价。
...
/**
 * Collects read-only evidence for the 股票估值与投资价值 procedure.
...
function collectEvidence(state: AdvisorState, context: AdvisorContext): Promise<ToolMessage[]> {
  ...
  return evidence.collectAll([
    ...
    evidence.callReports(context, target.display + " 投资价值 估值", evidence.symbolsFor([target]), null),
  ]);
}
