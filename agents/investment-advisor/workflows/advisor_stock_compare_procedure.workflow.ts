...
const PROCEDURE = "advisor_stock_compare_procedure" as const;
...
patch({
  progress: "正在理解股票对比问题",
  state: {
    status: AdvisorStatusSchema.describe("Only set cancelled when the user explicitly stops this 股票对比 workflow; otherwise set researching when the latest message asks or changes this procedure's research request."),
  ...
  instruction: `
本 workflow 只处理多股票、同行业或跨行业对比问题。
抽取至少两个对比标的到 targets，比较维度到 comparisonFocus；targetKind 设为 stock。
...
effect("resolveResearchReadiness", ["status", "targets", "topic"], {
  description: "股票对比 workflow 根据已抽取的研究对象和主题判断是否具备资料收集条件；不调用外部工具。",
  run: (state) => {
...
effect("collectResearchEvidence", [
  ...
  description: "股票对比 workflow 调用本 procedure 需要的只读投研工具；工具结果只通过 ToolMessage 暴露给 render，不写长期 state。",
  run: async (state, context, _runtime, step) => {
    if (state.status !== "researching" || readinessBlocker(state) !== null) return {};

    const loading = step.start("收集股票对比资料");
...
少于两个标的时必须要求补充另一个对比对象。
跨行业标的要说明周期属性、商业模式、估值体系或监管环境差异，不能强行给单一胜负结论。
...
/**
 * Collects read-only evidence for the 股票对比 procedure.
...
function collectEvidence(state: AdvisorState, context: AdvisorContext): Promise<ToolMessage[]> {
  const targets = evidence.normalizedTargets(state);
  ...
  return evidence.collectAll([
    ...
    evidence.callReports(
      context,
      targets.map((target) => target.display).join(" ") + " " + (state.comparisonFocus ?? "对比"),
      evidence.symbolsFor(targets),
      null,
    ),
  ]);
}
...
function readinessBlocker(state: AdvisorState): z.infer<typeof BlockerSchema> | null {
  if (true && state.targets.length < 2) return "insufficient_compare_targets";
