...
const PROCEDURE = "advisor_stock_event_research_procedure" as const;
...
const metadata = {
  id: PROCEDURE,
  version: "1.0.0",
  description: "投资顾问股票事件研究，处理处罚复产公告新闻研报机构观点。",
  routing: defineRouting({
    examples: ["ST未名的子公司复产需要满足哪些条件","这家公司被处罚有什么影响","宁德时代最新公告怎么看"],
    entities: ["处罚","复产","公告","新闻","研报","机构观点","事件"],
...
patch({
  progress: "正在理解股票事件问题",
  state: {
    status: AdvisorStatusSchema.describe("Only set cancelled when the user explicitly stops this 股票事件研究 workflow; otherwise set researching when the latest message asks or changes this procedure's research request."),
  ...
  instruction: `
本 workflow 只处理处罚、复产、公告、新闻、研报、机构观点等事件研究问题。
抽取公司或股票到 targets，事件主题到 topic；targetKind 通常设为 stock。
...
effect("resolveResearchReadiness", ["status", "targets", "topic"], {
  description: "股票事件研究 workflow 根据已抽取的研究对象和主题判断是否具备资料收集条件；不调用外部工具。",
  run: (state) => {
...
effect("collectResearchEvidence", [
  ...
  description: "股票事件研究 workflow 调用本 procedure 需要的只读投研工具；工具结果只通过 ToolMessage 暴露给 render，不写长期 state。",
  run: async (state, context, _runtime, step) => {
    if (state.status !== "researching" || readinessBlocker(state) !== null) return {};

    const loading = step.start("收集股票事件研究资料");
...
export default render({
  ...
  progress: "正在生成事件研究回复",
  instruction: `
本 workflow 的回复目标是复述可验证事实、梳理条件和影响。
必须优先说明事件背景、已披露事实、关键条件、可能时间线、相关主体和待核验事项。
缺少公告原文、监管文件或公司正式披露时，必须提示需要进一步核验，不能把媒体报道、研报推断或传闻当成确定结论。
如果工具事实没有返回目标公司公告、新闻或研报中的明确原文/摘要，必须先声明“当前可得资料未返回该事件的公司正式披露或公告原文”；随后只能给一般监管/业务条件框架，并把它标为待核验框架，不能写成该公司已经披露的具体复产条件。
引用条件、时间线、监管要求或主体关系时，必须说明它来自工具资料、公告摘要、新闻/研报，还是一般规则推导；来源不清时归入“仍需核验”。
...
/**
 * Collects read-only evidence for the 股票事件研究 procedure.
...
function collectEvidence(state: AdvisorState, context: AdvisorContext): Promise<ToolMessage[]> {
  ...
  return evidence.collectAll([
    ...evidence.companyEvidence(context, target, ["profile"]),
    evidence.callNews(context, query, 10),
    evidence.callReports(context, query, evidence.symbolsFor([target]), null),
  ]);
}
