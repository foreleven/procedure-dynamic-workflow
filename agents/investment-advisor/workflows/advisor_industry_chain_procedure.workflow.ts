...
const PROCEDURE = "advisor_industry_chain_procedure" as const;
...
const metadata = {
  id: PROCEDURE,
  version: "1.0.0",
  description: "投资顾问产业链研究，处理产业链上下游相关 A 股公司代表性公司梳理。",
  routing: defineRouting({
    examples: ["人形机器人产业链上下游企业有哪些","光伏产业链相关A股公司","算力产业链代表公司"],
    entities: ["产业链","上下游","相关A股","相关 A 股","代表公司"],
...
patch({
  progress: "正在理解产业链问题",
  state: {
    status: AdvisorStatusSchema.describe("Only set cancelled when the user explicitly stops this 产业链梳理 workflow; otherwise set researching when the latest message asks or changes this procedure's research request."),
  ...
  instruction: `
本 workflow 只处理产业链上下游、相关 A 股公司、代表性公司梳理。
抽取产业、行业、主题或公司到 topic/targets；targetKind 设为 industry。
...
effect("resolveResearchReadiness", ["status", "targets", "topic"], {
  description: "产业链梳理 workflow 根据已抽取的研究对象和主题判断是否具备资料收集条件；不调用外部工具。",
  run: (state) => {
...
effect("collectResearchEvidence", [
  ...
  description: "产业链梳理 workflow 调用本 procedure 需要的只读投研工具；工具结果只通过 ToolMessage 暴露给 render，不写长期 state。",
  run: async (state, context, _runtime, step) => {
    if (state.status !== "researching" || readinessBlocker(state) !== null) return {};

    const loading = step.start("收集产业链梳理资料");
...
export default render({
  ...
  progress: "正在生成产业链梳理回复",
  instruction: `
本 workflow 的回复目标是按上游、中游、下游、应用场景或关键零部件梳理产业结构。
列公司时必须在同一段写明：“以下公司仅为公开资料线索，信息需进一步核验，不是买入清单”。
不能凭空编造公司和产业链关系，不能把代表公司清单作为买入推荐。
...
/**
 * Collects read-only evidence for the 产业链梳理 procedure.
...
function collectEvidence(state: AdvisorState, context: AdvisorContext): Promise<ToolMessage[]> {
  ...
  const code = evidence.firstTargetCode(state);
  return evidence.collectAll([
    ...(code ? [evidence.call(context, "connectors.investmentAdvisor.getIndustryChainUpstreamDownstream", { code, src: null })] : []),
    evidence.callHotConcepts(context, topic, 10),
    evidence.callSectorRelatedEtfs(context, code),
    evidence.callReports(context, topic + " 产业链 上下游 A股 公司", null, [topic]),
    evidence.callNews(context, topic + " 产业链 相关公司", 10),
  ]);
}
