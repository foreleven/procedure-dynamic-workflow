...
const PROCEDURE = "advisor_sector_intraday_procedure" as const;
...
const metadata = {
  id: PROCEDURE,
  version: "1.0.0",
  description: "投资顾问板块短线研究，处理某行业或板块今日近期行情。",
  routing: defineRouting({
    examples: ["今天的券商板块股票如何","半导体板块近期表现","医药行业今天行情"],
    entities: ["板块","今日行情","近期表现","今天行情","短线"],
...
patch({
  progress: "正在理解板块行情问题",
  state: {
    status: AdvisorStatusSchema.describe("Only set cancelled when the user explicitly stops this 板块行情 workflow; otherwise set researching when the latest message asks or changes this procedure's research request."),
  ...
  instruction: `
本 workflow 只处理某行业或板块今日、近期行情。
抽取行业或板块到 topic/targets，时间到 horizon；targetKind 设为 sector。
...
effect("resolveResearchReadiness", ["status", "targets", "topic"], {
  description: "板块行情 workflow 根据已抽取的研究对象和主题判断是否具备资料收集条件；不调用外部工具。",
  run: (state) => {
...
effect("collectResearchEvidence", [
  ...
  description: "板块行情 workflow 调用本 procedure 需要的只读投研工具；工具结果只通过 ToolMessage 暴露给 render，不写长期 state。",
  run: async (state, context, _runtime, step) => {
    if (state.status !== "researching" || readinessBlocker(state) !== null) return {};

    const loading = step.start("收集板块行情资料");
...
export default render({
  ...
  progress: "正在生成板块行情回复",
  instruction: `
本 workflow 的回复目标是说明板块强弱、驱动和短线风险。
如果用户问今天/今日但工具事实不是同日盘中数据，必须说明资料是近期或存在时点限制，不能写成“今日盘中”。
没有板块指数或成分股结构化行情时，要说明分析主要来自热点、新闻和研报资料。
说明资料边界时只使用业务话术，例如“当前可得资料未返回板块指数或成分股盘中行情”；不要说“我不是实时工具”“没有结构化表格”“系统限制”“接口状态”等内部实现或工具状态。
即使资料有限，也要先围绕用户点名板块给出可用的强弱判断、驱动线索和短线风险，再补充资料边界。
不能把新闻/研报片段里的指数点位、涨跌幅、成交额或日期改写成今日板块行情；缺少结构化板块/成分股行情时，只能定性描述强弱和驱动。
如果没有当日盘中数据，仍需用“近期可得资料”回答板块强弱、驱动和短线风险，不能只停留在缺数据说明；明确写成“以下按近期线索而非今日盘中行情理解”。
禁止输出未来财报期、未来季度业绩、未经工具事实验证的并购/重组案例、估值分位数等精确事实。
...
/**
 * Collects read-only evidence for the 板块行情 procedure.
...
function collectEvidence(state: AdvisorState, context: AdvisorContext): Promise<ToolMessage[]> {
  ...
  return evidence.collectAll([
    evidence.callHotConcepts(context, topic, 10),
    evidence.callSectorRelatedEtfs(context, evidence.firstTargetCode(state)),
    evidence.callNews(context, topic + " 板块 今日 行情", 10),
    evidence.callReports(context, topic + " 板块 行情 资金流", null, [topic]),
  ]);
}
