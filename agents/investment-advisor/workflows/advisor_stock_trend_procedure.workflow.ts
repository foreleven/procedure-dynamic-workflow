...
const PROCEDURE = "advisor_stock_trend_procedure" as const;
...
const metadata = {
  id: PROCEDURE,
  version: "1.0.0",
  description: "投资顾问股票趋势研究，处理后市未来走势短期长期明天明日预测。",
  routing: defineRouting({
    examples: ["紫金矿业明日预测","宁德时代后市怎么看","贵州茅台未来走势"],
    entities: ["明日","明天","后市","后市会怎么走","走势","趋势","预测"],
...
patch({
  progress: "正在理解股票趋势问题",
  state: {
    status: AdvisorStatusSchema.describe("Only set cancelled when the user explicitly stops this 股票趋势研判 workflow; otherwise set researching when the latest message asks or changes this procedure's research request."),
  ...
  instruction: `
本 workflow 只处理后市、未来走势、短期/长期、明天/明日涨跌、趋势预测等问题。
抽取股票到 targets，周期到 horizon，日期范围到 dateRange；targetKind 设为 stock。
...
effect("resolveResearchReadiness", ["status", "targets", "topic"], {
  description: "股票趋势研判 workflow 根据已抽取的研究对象和主题判断是否具备资料收集条件；不调用外部工具。",
  run: (state) => {
...
effect("collectResearchEvidence", [
  ...
  description: "股票趋势研判 workflow 调用本 procedure 需要的只读投研工具；工具结果只通过 ToolMessage 暴露给 render，不写长期 state。",
  run: async (state, context, _runtime, step) => {
    if (state.status !== "researching" || readinessBlocker(state) !== null) return {};

    const loading = step.start("收集股票趋势研判资料");
...
export default render({
  ...
  progress: "正在生成趋势研判回复",
  instruction: `
本 workflow 的回复目标是做趋势情景推演和风险提示。
必须区分短期波动和中长期驱动，说明强势延续、震荡回落、风险释放等条件。
不能保证明日涨跌，不能承诺目标价或持有周期。
涉及“后市”“明日”“预测”“会怎么走”时，必须在趋势分析段明确写出短期走势不确定、不能保证涨跌；不要把“跌破支撑”“震荡磨底”“反转确认”等技术判断写成确定路径。
工具资料冲突时，要说明冲突来源和待核验事项。
...
/**
 * Collects read-only evidence for the 股票趋势研判 procedure.
...
function collectEvidence(state: AdvisorState, context: AdvisorContext): Promise<ToolMessage[]> {
  ...
  return evidence.collectAll([
    ...
    ...evidence.historicalEvidence(context, target, state),
    evidence.callNews(context, target.display + " 后市 趋势", 8),
    evidence.callReports(context, target.display + " 趋势 展望", evidence.symbolsFor([target]), null),
  ]);
}
