...
const PROCEDURE = "advisor_stock_financial_procedure" as const;
...
const metadata = {
  id: PROCEDURE,
  version: "1.0.0",
  description: "投资顾问股票财务研究，处理财务状况财报业绩现金流盈利能力营收增长。",
  routing: defineRouting({
    examples: ["贵州茅台2025Q1业绩","比亚迪财务状况怎么样","宁德时代现金流分析"],
    entities: ["财务","财报","业绩","现金流","盈利能力","营收"],
...
patch({
  progress: "正在理解股票财务问题",
  state: {
    status: AdvisorStatusSchema.describe("Only set cancelled when the user explicitly stops this 股票财务分析 workflow; otherwise set researching when the latest message asks or changes this procedure's research request."),
  ...
  instruction: `
本 workflow 只处理财务状况、财报、业绩、现金流、盈利能力、营收增长等问题。
抽取股票到 targets，财报期间到 financialPeriod；targetKind 设为 stock。
...
effect("resolveResearchReadiness", ["status", "targets", "topic"], {
  description: "股票财务分析 workflow 根据已抽取的研究对象和主题判断是否具备资料收集条件；不调用外部工具。",
  run: (state) => {
...
effect("collectResearchEvidence", [
  ...
  description: "股票财务分析 workflow 调用本 procedure 需要的只读投研工具；工具结果只通过 ToolMessage 暴露给 render，不写长期 state。",
  run: async (state, context, _runtime, step) => {
    if (state.status !== "researching" || readinessBlocker(state) !== null) return {};

    const loading = step.start("收集股票财务分析资料");
...
export default render({
  ...
  progress: "正在生成财务分析回复",
  instruction: `
本 workflow 的回复目标是解释经营质量和财务线索。
必须围绕收入结构、盈利能力、成长性、现金流、行业地位、财报期间和风险因素展开。
如果没有结构化财务报表，必须写“数据边界与风险”，不能编造营收、利润、现金流、毛利率或同比增速。
...
/**
 * Collects read-only evidence for the 股票财务分析 procedure.
...
function collectEvidence(state: AdvisorState, context: AdvisorContext): Promise<ToolMessage[]> {
  ...
  return evidence.collectAll([
    ...evidence.companyEvidence(context, target, ["profile", "mainBusiness", "industryRank"]),
    evidence.callReports(context, target.display + " " + (state.financialPeriod ?? "") + " 财务 业绩 现金流", evidence.symbolsFor([target]), null),
    evidence.callNews(context, target.display + " " + (state.financialPeriod ?? "") + " 财报 业绩", 8),
  ]);
}
