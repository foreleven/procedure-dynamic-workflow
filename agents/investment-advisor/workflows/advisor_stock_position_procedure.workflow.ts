...
const PROCEDURE = "advisor_stock_position_procedure" as const;
...
const metadata = {
  id: PROCEDURE,
  version: "1.0.0",
  description: "投资顾问股票仓位策略研究，处理买入卖出建仓加仓离场持有止盈止损。",
  routing: defineRouting({
    examples: ["圣邦股份股票现在可以进吗","比亚迪现在能不能买","比亚迪要不要加仓","隆基绿能止损还是持有"],
    entities: ["买入","卖出","建仓","加仓","离场","持有","止盈","止损","可以进","能不能买","要不要买"],
...
patch({
  progress: "正在理解仓位策略问题",
  state: {
    status: AdvisorStatusSchema.describe("Only set cancelled when the user explicitly stops this 股票仓位策略 workflow; otherwise set researching when the latest message asks or changes this procedure's research request."),
  ...
  instruction: `
本 workflow 只处理买入、卖出、建仓、加仓、离场、持有、止盈止损、能不能买、要不要卖等仓位策略问题。
抽取股票到 targets，交易意图到 action，周期到 horizon；targetKind 设为 stock。
...
effect("resolveResearchReadiness", ["status", "targets", "topic"], {
  description: "股票仓位策略 workflow 根据已抽取的研究对象和主题判断是否具备资料收集条件；不调用外部工具。",
  run: (state) => {
...
effect("collectResearchEvidence", [
  ...
  description: "股票仓位策略 workflow 调用本 procedure 需要的只读投研工具；工具结果只通过 ToolMessage 暴露给 render，不写长期 state。",
  run: async (state, context, _runtime, step) => {
    if (state.status !== "researching" || readinessBlocker(state) !== null) return {};

    const loading = step.start("收集股票仓位策略资料");
...
export default render({
  ...
  progress: "正在生成仓位策略回复",
  instruction: `
本 workflow 的回复目标是提供条件化仓位策略和风险控制。
必须给入场、等待、离场、止损或观察条件，不能输出“可以买”“必须卖”“现在加仓”这类无条件指令。
涉及补仓、追高、止损时，要说明放大亏损、趋势未确认或事件风险。
只能给风险管理框架，不能替用户做个性化资产配置。
...
/**
 * Collects read-only evidence for the 股票仓位策略 procedure.
...
function collectEvidence(state: AdvisorState, context: AdvisorContext): Promise<ToolMessage[]> {
  ...
  const query = evidence.strategyQuery(state, target.display);
  return evidence.collectAll([
    ...evidence.stockMarketEvidence(context, target),
    evidence.callNews(context, query, 8),
