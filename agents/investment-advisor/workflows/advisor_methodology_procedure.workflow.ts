...
const PROCEDURE = "advisor_methodology_procedure" as const;
...
patch({
  progress: "正在理解研究方法论问题",
  state: {
    status: AdvisorStatusSchema.describe("Only set cancelled when the user explicitly stops this 投资研究方法论 workflow; otherwise set researching when the latest message asks or changes this procedure's research request."),
  ...
  instruction: `
本 workflow 只处理方法论、步骤范本、预测模型、成功案例、政策方案等问题。
抽取研究对象或方法主题到 topic；targetKind 设为 methodology。
...
effect("resolveResearchReadiness", ["status", "targets", "topic"], {
  description: "投资研究方法论 workflow 根据已抽取的研究对象和主题判断是否具备资料收集条件；不调用外部工具。",
  run: (state) => {
...
effect("collectResearchEvidence", [
  ...
  description: "投资研究方法论 workflow 调用本 procedure 需要的只读投研工具；工具结果只通过 ToolMessage 暴露给 render，不写长期 state。",
  run: async (state, context, _runtime, step) => {
    if (state.status !== "researching" || readinessBlocker(state) !== null) return {};

    const loading = step.start("收集投资研究方法论资料");
...
必须提供方法步骤、指标清单、数据解释方式、交叉验证办法、适用范围和风险注意事项。
示例和模板不能包含编造的精确百分比、利用率、市场规模增量或利润改善；使用占位符或提示用实际数据代入。
没有工具资料支持成功案例时，不能假装存在确定成功案例。
...
/**
 * Collects read-only evidence for the 投资研究方法论 procedure.
...
function collectEvidence(state: AdvisorState, context: AdvisorContext): Promise<ToolMessage[]> {
  ...
  return evidence.collectAll([
    evidence.callReports(context, topic + " 方法论 范本 模型 指标", null, null),
    evidence.callNews(context, topic + " 方法 指标 数据 解读", 8),
  ]);
}
