...
import { createConnectorRegistry, z } from "@pac/workflow";
import {
  createLlmClient,
  WorkflowEngine,
...
import loadConnectorTools from "./connectors/main.js";
...
const investmentAdvisorWorkflows = [
  stockBriefWorkflow,
...
const workflowsById = new Map(
  investmentAdvisorWorkflows.map((workflow) => [workflow.id, workflow]),
);
...
const connectors = createConnectorRegistry(loadConnectorTools());
...
console.log(
  `Investment advisor workflow score: ${score.score}/100 ${score.grade}`,
);
...
async function runCase(testCase: WorkflowCase): Promise<CaseRecord> {
  ...
  const unknownWorkflowIds = workflowIds.filter(
    (workflowId) => !workflowsById.has(workflowId),
  );
  if (unknownWorkflowIds.length > 0) {
    throw new Error(
      `No investment advisor workflow registered for case ${testCase.id}: ${unknownWorkflowIds.join(", ")}`,
    );
  }
  ...
  for (const [index, turn] of testCase.turns.entries()) {
    ...
    try {
      ...
      const judgement = await judgeResponse({
        ...
        responseWorkflowIds: result.responses.map(
          (response) => response.workflowId,
        ),
      ...
      console.log(
        `${testCase.id} turn ${index + 1}: ${judgement.verdict} - ${judgement.reason} (${formatDuration(durationMs)})`,
      );
    } catch (error) {
      ...
      console.log(
        `${testCase.id} turn ${index + 1}: fail - runtime error (${formatDuration(durationMs)})`,
      );
    }
  }
...
function scoreRun(records: CaseRecord[]): ScoreSummary {
  ...
  const severeCount = allBugSignals.filter((signal) =>
    severeSignals.has(signal),
  ).length;
  ...
  const businessSafety = Math.max(
    0,
    safetyBase - severeCount * 8 - failTurns * 2 - warnTurns,
  );
  const conversationExperience = Math.max(
    0,
    Math.round(10 * weightedTurnRatio) - Math.max(0, failTurns - runtimeErrors),
  );
...
function writeReport(
  ...
  const reportPath = resolve(
    reportsDir,
    `${timestampForId(started)}_${sessionPrefix}.md`,
  );
  writeFileSync(
    reportPath,
    reportMarkdown(records, score, started, finished),
    "utf8",
  );
  return reportPath;
}
...
function compactAdvisorStates(
  session: Parameters<WorkflowEngine["getWorkflowSnapshot"]>[0],
  workflowIds: string[],
): CompactAdvisorState[] {
  const states: CompactAdvisorState[] = [];
  for (const workflowId of workflowIds) {
    const snapshot = engine.getWorkflowSnapshot<InvestmentAdvisorRuntimeState>(
      session,
      workflowId,
    );
    if (snapshot) {
...
function compactAdvisorState(
  procedure: string,
  state: InvestmentAdvisorRuntimeState & { messages: unknown[] },
): CompactAdvisorState {
  return {
    status: state.status,
    procedure,
...
function loadCases(): WorkflowCase[] {
  ...
  return WorkflowCasesFileSchema.parse(
    YAML.parse(readFileSync(yamlPath, "utf8")),
  ).cases;
}
