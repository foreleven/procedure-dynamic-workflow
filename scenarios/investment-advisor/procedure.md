...
用户进入投资顾问场景后，按业务意图分流到对应 procedure。一个问题同时包含多个明确意图时，应分别进入对应 workflow 并行收集资料和生成各自回复；例如“比亚迪现在能不能买，顺便看下估值贵不贵”可以同时进入仓位策略和估值研究。每个 workflow 只能处理自己的业务边界，可以引用其他资料作为补充，但不能把一个 procedure 的目标改写成另一个 procedure。
