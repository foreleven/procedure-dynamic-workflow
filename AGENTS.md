...
- `agent.yaml` 存稳定 metadata、routing、connector 文件名和验收 cases；CLI 通过 `workflows.<name>` 加载 `workflows/<name>.workflow.ts`，通过 `connectors` 文件名加载 `connectors/<name>.ts`。`*.workflow.ts` 是可分发 workflow artifact。
