...
- `agents/**` 是业务 workflow 示例与验证材料，不是稳定公共 API。
...
- 模型调用返回 workflow patch structured output 失败时，严禁在 engine、LLM client 或 CLI 里添加针对 patch 的重试逻辑；应修正 workflow patch schema、Zod 边界或 patch/render instructions，让一次结构化调用本身可解释、可校验。
- engine 应继续防御非序列化诊断值、重复 workflow id、未知 active workflow、无效 default state、无效 render response 和保留字段写入。

### `agents`

- 每个业务 agent 通常包含 `procedure.md`、`agent.yaml`、`workflows/*.workflow.ts`、`connectors/*.ts`、可选 `connectors/mockData.ts` 和验收 cases。
- `agent.yaml` 存稳定 metadata、routing、connector 文件名和验收 cases；CLI 通过 `workflows.<name>` 加载 `workflows/<name>.workflow.ts`，通过 `connectors` 文件名加载 `connectors/<name>.ts`。agent 目录下的 `*.workflow.ts` 导出 metadata-less workflow template，CLI 从 `agent.yaml` 注入 metadata 后再交给 engine。
- 示例代码可以服务演示和验证，但不要从示例反向发明新业务规则；新业务规则必须来自对应 procedure。

## Workflow 编写规则

写或改 PAC workflow 时，先读对应 `procedure.md`。procedure 是业务事实来源，`agent.yaml`、workflow 文件、connector 文件、mock data 和测试都应从它编译出来。

工作流文件应使用现有 program DSL：

1. `const { patch, prefetch, effect, command, render } = workflow<State, ConnectorCatalog>({ stateSchema, state })`
2. 先声明 `patch(...)`
3. 再声明 `prefetch(...)`
4. 再声明 `effect(...)`
5. 再声明 `command(...)`
...
- agent workflow 代码不要声明或加载 `id`、`version`、`description`、`routing`；这些稳定 metadata 只在 `agent.yaml` 里声明。只有脱离 agent manifest 的 standalone workflow module 才可以在 `workflow(...)` 里直接传 metadata。
...
- workflow/agent 改动优先补确定性 runtime 测试：fake structured LLM、固定时钟、mock connector、断言运行完成后的业务 state、context、tool messages 和 command side effects。
...
- `npm run chat -- agents/maintenance --all-cases --no-stream` 和 `npm run test:llm` 依赖真实 LLM，适合手动 smoke test。运行或跳过都要在最终说明中讲清楚原因。
...
- 相关 README、`docs/API.md`、agent procedure、workflow metadata 或 skill 文档已同步。
